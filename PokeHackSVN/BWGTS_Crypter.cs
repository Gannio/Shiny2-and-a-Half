...
namespace BW
{
    public static class Pokemon
    {
        ...
        private static bool mDictionariesInitialized;
        ...
        {
            var checksum = CalculateChecksum(pkm);
        ...
        public static UInt16 CalculateChecksum(byte[] pkm)
        {
            var data = (byte[]) pkm.Clone();

            uint index = 0;

            for (var i = 8; i <= 134; i += 2)
                index += (uint) (data[i] + (data[i + 1]*256));

            return Convert.ToUInt16(DecToBin(index, 32).Substring(16, 16), 2);
        ...
        private static byte[] ShuffleBytes(byte[] unencryptedData)
        {
            ...
            var pid = BitConverter.ToUInt32(unencryptedData, 0);
            var unShuffleIndex = (UInt16) (((pid >> 0xd) & 0x1f)%24);
            var shuffleOrder = dpPKMShuffle[unShuffleIndex];

            var Block1 = new byte[32];
            var Block2 = new byte[32];
            var Block3 = new byte[32];
            var Block4 = new byte[32];
        ...
        private static byte[] Decrypt_Data(byte[] Data)
        {
            ...
            for (int i = 8; i <= 135; i += 2)
            {
                ...
                var aft = (ushort) (bef ^ (prng.NextNum() >> 0x10));
